...
OUTPUT_FILE="${OUTPUT_DIR}/waf_report.json"
START_DATE=""
END_DATE=""
    ...
    if [[ "$(echo "$WAF_DATA" | jq '.WebACLs | length')" -gt 0 ]]; then
        echo "$WAF_DATA" | jq -c '.WebACLs[]' | while read -r web_acl; do
            ...
            jq -n \
                --arg name "$NAME" \
                --arg id "$ID" \
                --arg allowed_reqs "$ALLOWED_REQUESTS" \
                --arg blocked_reqs "$BLOCKED_REQUESTS" \
                ...
                '{
                    ...
                    "name": $name,
                    "id": $id,
                    "allowedRequests": ($allowed_reqs | tonumber),
                    "blockedRequests": ($blocked_reqs | tonumber),
